...
class Pktdblib {
    ...
    function _insert($data) {
        //echo '<pre>'; echo "reached in library";print_r($data);exit;
    ...
    function _update($id, $data, $updateColumn = NULL) {
        /*echo $id;
        print_r($data);exit;*/
       // $db = $this->database;
        $table = $this->get_table();
        ...
        if(NULL!==$updateColumn)
                $this->CI->db->where($updateColumn, $id);
        else
                $this->CI->db->where('id', $id);
    ...
    function get_condition_wise_data($conditions = []) {
