...
class Pktlib {
    ...
    function createSingleImageThumbs($pathToImages, $filename, $thumbArray ){
        ...
        if(isset($info['extension'])){
            ...
            $width = imagesx( $img );
            $height = imagesy( $img );
            if(isset($thumbArray['folder'])){
                //$this->CI->pktdblib->set_table('image_setup');
                $imageSetup = $this->CI->pktdblib->custom_query('Select * from image_setup where image_folder_name like "'.$thumbArray['folder'].'" and is_active=true');
                foreach ($imageSetup as $key => $setup) {
                    //$filename = '';
                    $filename2 = $setup['width'].'x'.$setup['height'].'_'.$filename;
                    $new_width = (isset($setup['width']) && $setup['width']!='x')?$setup['width']:$width;
                    $new_height = (isset($setup['height']) && $setup['height']!='x')?$setup['height']:floor( $height * ( $setup['width'] / $width ) );
                    //echo $new_height." ".$new_width."<br>";exit;
                    ...
                    imagejpeg( $tmp_img, "{$thumbArray['path']}{$filename2}" );
                }
                //print_r($imageSetup);
    ...
    function createThumbs( $pathToImages, $pathToThumbs, $thumbWidth, $folder='' ) 
    {
      // open the directory
            ...
            if(empty($img))
                ...
                {*/
                 // echo "Creating thumbnail for {$fname} <br />";
                  ...
                  if($folder!=''){
                    //$this->CI->pktdblib->set_table('image_setup');
                    $imageSetup = $this->CI->pktdblib->custom_query('Select * from image_setup where image_folder_name like "'.$folder.'" and is_active=true');
                    //print_r($imageSetup);
                    foreach ($imageSetup as $key => $setup) {
                        //$filename = '';
                        $filename2 = $setup['width'].'x'.$setup['height'].'_'.$fname;
                        echo $filename2."<br>";
                        $new_width = (trim(strtolower($setup['width']))!='x')?$setup['width']:$width;
                        $new_height = (trim(strtolower($setup['height']))!='x')?$setup['height']:floor( $height * ( $setup['width'] / $width ) );
                        ...
                        // save thumbnail into a file
                        imagejpeg( $tmp_img, "{$pathToThumbs}{$filename2}" );
                    }
                    //print_r($imageSetup);
    ...
    function create_nested_menu($parent = 1, $menuId = 1, $session_menu = []) {
        $items = array();
