    ...
    function tbl_product_categories(){
        ...
        if(!$check){
	        ...
	        if($query){
				...
				$productDocument = $this->db->query("CREATE TABLE IF NOT EXISTS `product_documents` (
					`id` int(11) NOT NULL AUTO_INCREMENT,
					`product_id` int(11) NOT NULL,
				...
				$productImages = $this->db->query("CREATE TABLE IF NOT EXISTS `product_images` (
					`id` int(11) NOT NULL AUTO_INCREMENT,
					`image_name_1` varchar(255) NOT NULL,
					`image_name_2` varchar(255) NOT NULL,
					`product_id` int(11) NOT NULL,
				...
				$productDetails = $this->db->query("CREATE TABLE IF NOT EXISTS `product_details` (
					`id` int(11) NOT NULL AUTO_INCREMENT,
					`product_id` int(11) NOT NULL,
...
//db->join($table.' c2', 'c2.parent_id='.$table.'.id', 'left');
		...
		$db->order_by($table.'.priority asc');
		//$db->limit(3, 0);
		$query = $db->get_where($table, $condition);
		//print_r($db->last_query());
		return $query->result_array();
