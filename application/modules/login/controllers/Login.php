...
class Login extends MY_Controller {
    ...
    function admin_index() {
		...
		if($this->input->post('username/email') or $this->input->post('password'))
		{
			...
			if($this->form_validation->run())
			{
				...
				exit;*/
				//echo $username.' '.$password." ".$type;exit;
	...
	private function _email_credentials($email, $type) {
		...
		$config = Array(
		    'protocol' => 'smtp',
		    'smtp_host' => custom_constants::mailer_host,
		    'smtp_port' => custom_constants::mailer_port,
		    'smtp_user' => custom_constants::mailer_address,
            'smtp_pass' => custom_constants::mailer_password,
            'charset'   => 'utf-8',
            'smtp_crypto' => 'ssl'
		);
...
*/ 
    ...
    function emailtest(){
        ...
        $this->email->from('primarykeytech@gmail.com', 'Test mail');
    ...
    function register_employee_to_login($data){
		...
		$id = $this->pktdblib->_insert($insert_data);
		$this->pktdblib->set_table('roles');
		$query = $this->pktdblib->get_where_custom('module', 'employees');
	...
	function register_company_to_login($data){
		...
		$id = $this->pktdblib->_insert($insert_data);
		$this->pktdblib->set_table('roles');
		$query = $this->pktdblib->get_where_custom('module', 'companies');
		$roles = $query->row_array();
		//print_r($roles);exit;
		$insert['user_id'] = $data['id'];
		$insert['role_id'] = $roles['id'];
	...
	function register_customer_to_login($data){
		...
		$userRoleId = $this->pktdblib->_insert($insert);
		//exit;
		return json_encode(["msg"=>"Login Successfully Inserted", "status"=>"success", 'id'=>$data['id']]);
	}
