...
class Event_model extends CI_Model {
    ...
    function tbl_event_categories(){
		...
		if($query){
				$events = $this->db->query("CREATE TABLE IF NOT EXISTS `events` (
						`id` int(11) NOT NULL AUTO_INCREMENT,
				...
				$events_category_section = $this->db->query("CREATE TABLE IF NOT EXISTS `events_category_sections` (
					  `id` int(11) NOT NULL AUTO_INCREMENT,
				...
				$blogCities = $this->db->query("CREATE TABLE IF NOT EXISTS `events_cities` (
					`id` int(11) NOT NULL AUTO_INCREMENT,
	...
	function get_events_category_list($id=null) {
		$db = $this->database;
		$condition['is_active'] = true;
		
		$db->select('id,category_name')->where('`id` NOT IN (SELECT event_category_id from `events`)');
		$query = $db->get_where('event_categories', $condition);
		//print_r($db->last_query());
		return $query->result_array();
	}

	function get_distinct_categorylist_for_events() {
		$condition['is_active'] = true;
		$db = $this->database;
		$table = $this->get_table();
		$db->select(['DISTINCT(event_categories.id)', 'event_categories.category_name']);
		$db->join('events', 'events.events_category_id = event_categories.id');
		$query = $db->get($table);
		return $query->result_array(); 
		}
	...
	function get_city_list($id=null) {
		$db = $this->database;
		$condition['events_cities.is_active'] = true;
		$condition['events_cities.events_id'] = $id;
		$db->select(['events_cities.*', 'cities.city_name']);
		//$db->select(['events.*', 'states.state_name']);
		$db->join('cities', 'cities.id = events_cities.city_id');
		//$db->join('states', 'states.id = events.state_id');

		//$db->join()
		//$db->select('id,category_name')->where('`id` NOT IN (SELECT events_category_id from `events`)');
		$query = $db->get_where('events_cities', $condition);
		//print_r($db->last_query());
		return $query->result_array();
	}
	...
	function get_state_list($id=null) {
		$db = $this->database;
		$condition['events.is_active'] = true;
		$condition['events.id'] = $id;
		$db->select(['events.*', 'states.state_name']);
		//$db->select(['events.*', 'states.state_name']);
		$db->join('states', 'states.id = events.state_id');
		//$db->join('states', 'states.id = events.state_id');
		...
		//$db->join()
		//$db->select('id,category_name')->where('`id` NOT IN (SELECT events_category_id from `events`)');
		$query = $db->get_where('events', $condition);
		//print_r($db->last_query());
		return $query->result_array();
	}
...
//db->join($table.' c2', 'c2.parent_id='.$table.'.id', 'left');
	...
	function get_categorylist_for_events() {
		$db = $this->database;
		$condition['is_active'] = true;
		//$sql = 'Select id, category_name from event_categories where id in (Select distinct events_category_id from events) union Select id, category_name from event_categories where id Not in (Select distinct events_category_id from events)';
	...
	function get_active_list($id = NULL) {
		...
		if (empty($id))
        {
            ...
            $query = $this->db->get_where('events', $condition);
            return $query->result_array();
        }
        $condition['id'] = $id;
        $query = $this->db->get_where('events', $condition);
	...
	function get_events_list($conditions){
		$db = $this->database;
		$table = $this->get_table();
		//$condition['events.id'] = $id;
		//echo $conditons;exit;
		//$db->select(['products.*', 'product_categories.category_name', 'product_images.image_name_1', 'product_images.image_name_2']);
		$db->select(['events.*', 'event_categories.category_name', 'login.first_name', 'login.surname', 'states.state_name']);
		$db->join('event_categories', 'event_categories.id = events.event_category_id', 'left');
		$db->join('login', 'login.employee_id = events.user_id and account_type="employees"', 'left');
		$db->join('states', 'states.id = events.state_id', 'left');
	...
	function delete($id) {
		...
		$db = $this->database;
		$table = $this->get_table();
		$query = $db->delete($table, array('events_id' => $id));
		return $query;
    }
}
