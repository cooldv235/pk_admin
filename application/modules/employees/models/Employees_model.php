  ...
  UNIQUE KEY (primary_email)
        ...
        if($query){
            ...
            if($menuId){
              ...
              $roles = $this->db->insert('roles', ['module'=>'employees', 'role_name'=>'Employees', 'role_code'=>'emp', 'created'=>date('Y-m-d H:i:s'), 'modified'=>date('Y-m-d H:i:s')]);
              if($roles){
                $roleId =  $this->db->insert_id();
                $rolesDetails = $this->db->insert('role_details', ['role_id'=>$roleId, 'is_view'=>0, 'is_add'=>1, 'is_update'=>1, 'is_delete'=>0, 'created'=>date('Y-m-d H:i:s'), 'modified'=>date('Y-m-d H:i:s')]);
              }
