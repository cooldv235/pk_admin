...
class Address extends MY_Controller {
	...
	function admin_edit_form($data = []) {
		...
		if(isset($data['module']) && $data['module']=='companies'){
			...
			//show_404();
			//exit;
			/*if($data['address']['type']=='login'){
				$this->pktdblib->set_table('user_roles');
				$roles = $this->pktdblib->get_where_custom('login_id', $data['address']['user_id']);
	...
	function type_wise_user2($data = []){
		...
		$sql = 'Select a.id,a.site_name from customers c inner join user_roles ur on ur.user_id=c.id and ur.account_type="'.$userType.'" INNER join address a on a.user_id=ur.login_id and a.type="login" where ur.user_id='.$userId.' and a.is_active=true';

		$typeWiseUsers = $this->pktdblib->custom_query($sql);
		$userList = [0=>['id'=>0, 'text'=>'Select Address']];
		foreach ($typeWiseUsers as $key => $typeWiseUser) {
			$userList[$key+1]['id'] = $typeWiseUser['id'];
			$userList[$key+1]['text'] = $typeWiseUser['site_name'];
		}
