...
class Blogs_model extends CI_Model {
    ...
    function tbl_blogs_categories(){
    	$check = $this->check_table('blogs_categories');
    	//print_r($check);exit;
    	if(!$check){
    		$query = $this->db->query("CREATE TABLE IF NOT EXISTS `blogs_categories` (
...
//db->join($table.' c2', 'c2.parent_id='.$table.'.id', 'left');
	...
	function get_blogs_list($conditions){
		...
		$db->select(['blogs.*', 'blogs_categories.category_name', 'login.first_name', 'login.surname', 'states.state_name']);
		$db->join('blogs_categories', 'blogs_categories.id = blogs.blogs_category_id', 'left');
