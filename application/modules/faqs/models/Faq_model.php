...
class Faq_model extends CI_Model {
	...
	function get_distinct_category_dropdown_list($id = NULL) {
		$db = $this->database;
		//$condition['is_active'] = TRUE;
		...
		//$db->select('id,faq_category');
		$db->select('DISTINCT(faq_category)');
		/*$db->from('faques');*/
        $query = $db->get('faques');
	...
	function get_active_list($id = NULL) {
		...
		if (empty($id))
        {
            ...
            $query = $this->db->get_where('faques', $condition);
            return $query->result_array();
        }
        $condition['id'] = $id;
        $query = $this->db->get_where('faques', $condition);
	...
	/*function get_faq_list($conditions){
    	$db = $this->database;
		$table = $this->get_table();
		//print_r($table);
		$db->select($table.'.*, faques.faq_category as faq_category');
		$db->join('documents', 'documents.id='.$table.'.document_id');
