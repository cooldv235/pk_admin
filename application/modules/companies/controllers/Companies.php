...
class Companies extends MY_Controller {
	...
	function get_all_company_list() {
		...
		$query = $this->pktdblib->get('modified desc');
	...
	function admin_edit($id) {
		...
		/*print_r($userRoles);
		exit;*/
		/*$AddressListData = ['url'=>custom_constants::edit_company_url.'/'.$id.'?tab=address', 'condition'=>['address.user_id'=>$loginId, 'address.type'=>'login'], 'module'=>'companies'];*/
		$AddressListData = ['url'=>custom_constants::edit_company_url.'/'.$id.'?tab=address', 'condition'=>['address.user_id'=>$loginId, 'address.type'=>'login'], 'module'=>'companies'];
		...
		$data['otherDetails'] = Modules::run("companies/admin_edit_other_details", $id);//exit;
	...
	function admin_edit_other_details($companyId) {
    	//echo $productId;
 		if($_SERVER['REQUEST_METHOD'] == 'POST'){ 
 			/*echo '<pre>';
 			print_r($_POST);
			...
			$this->form_validation->set_error_delimiters('<span class="text-danger">','</span>');
			$this->form_validation->set_rules("data[company_details][mission]", 'Mission', 'required');
			$this->form_validation->set_rules("data[company_details][vission]", 'Vision', 'required');
			$this->form_validation->set_rules("data[company_details][team_text]", 'Team Text', 'required');
			
			if($this->form_validation->run('data[company_details]')!== FALSE){
				$postData = $this->input->post('data[company_details]');
				$error = [];
				if(!empty($_FILES['image']['name'])) {
					$imageFileValidationParams = ['file'=>$_FILES['image'], 'path'=>'../content/uploads/team/', 'fieldname'=>'image', 'ext'=>'jpeg|png|jpg|gif', 'arrindex'=>'image'];
					//print_r($imageFileValidationParams);exit;
					$imageImg = $this->pktlib->upload_single_file($imageFileValidationParams);
					//print_r($imageImg);exit;
					if(empty($imageImg['error'])) {
						$postData['image'] = $imageImg['filename'];
						unset($postData['image2']);
					}
					else {
						$error['image'] = $imageImg['error'];
					}
				}
				else {
					$postData['image'] = $postData['image2'];
					unset($postData['image2']);
				}
				//print_r($postData);exit;
				$postData['modified'] = date('Y-m-d H:i:s');
				...
				$flag = false;
				$this->pktdblib->set_table('company_details');
				if(!empty($insert))
				...
				if($flag){
					...
					$this->session->set_flashdata('message', $msg);
					redirect('companies/editcompany/'.$companyId.'?tab=other_details');
				}else{
 			...
 			$this->pktdblib->set_table('company_details');
 			$companyDetails = $this->pktdblib->get_where_custom('company_id', $companyId);
 			$data['values_posted']['company_details'] = $companyDetails->row_array();
 		}
 		$data['id'] = $companyId;
 		
 		//print_r($data['product_images']);
 		$this->load->view('companies/admin_edit_other_details', $data);
    }
}